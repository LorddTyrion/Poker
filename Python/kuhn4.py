...
def get_terminal_utility(history, cards, fixed_player):
    # Return utility from the fixed player's perspective
    if history == "pp":  # Showdown after checks
        return 1 if card_rank[cards[fixed_player]] > card_rank[cards[1-fixed_player]] else -1
        ...
        return 2 if card_rank[cards[fixed_player]] > card_rank[cards[1-fixed_player]] else -2
...
class CFR:
    def cfr(self, cards, history, fixed_player, p0, p1):
        ...
        if is_terminal(history):
            return get_terminal_utility(history, cards, fixed_player)
...
def train(iterations):
    cards = ['J', 'Q', 'K']
    cfr = CFR()
    #card_deals = [(cards[i], cards[j]) for i in range(3) for j in range(3) if i != j]
    for i in range(iterations):
        ...
        # for player1_card, player2_card in card_deals:
        #     cfr.cfr([player1_card, player2_card], "", 0, 1, 1)
        #     cfr.cfr([player1_card, player2_card], "", 1, 1, 1)
...
def play_against_agent():
    ...
    payoff = get_terminal_utility(history, [agent_card, human_card], 0)  # Get payoff from agent's perspective
    human_payoff = -payoff
...
def analyze_nash_equilibrium():
    ...
    # Player 1 strategies
    p1_j_check = node_map["J"].get_average_strategy()[0] if "J" in node_map else "N/A"
    p1_q_bet = node_map["J"].get_average_strategy()[1] if "J" in node_map else "N/A"
    ...
    p1_k_call = node_map["Kpb"].get_average_strategy()[0] if "Kpb" in node_map else "N/A" 
...
train(1000000)  # Running fewer iterations since we run CFR twice per iteration
